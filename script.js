...
function setupGallerySlider() {
    ...
    const counterNow = document.getElementById('galleryCounterNow');
    ...
    function playKenBurns() {
        if (kenTween) { kenTween.kill(); kenTween = null; }
        const img = slides[current].querySelector('.gallery-img');
        if (!img) return;
        kenTween = gsap.fromTo(img, { scale: 1.14 }, {
            scale: 1,
            duration: SLIDE_MS / 1000 + 1,
            ease: 'power1.out'
        });
    }

    function revealSlideFx() {
        const slide = slides[current];
        const caption = slide.querySelector('.gallery-caption');
        const logo = slide.querySelector('.gallery-logo');
        if (caption) {
            gsap.fromTo(caption,
                { y: 26, opacity: 0 },
